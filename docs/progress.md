...
**What Changed**:
- All 56 source files updated: imports, `go.mod` declarations, README, docs
...
**Resolution**:
1. Deleted old tags (`agent/v0.1.0`, `v0.1.0`) from the `superbuilders` remote
2. Bumped version to `v0.2.0` in root `go.mod`
3. Added `replace github.com/superbuilders/clyde/agent v0.2.0 => ./agent` directive for local dev
4. Workspace (`go.work`) still used, but `replace` is the actual mechanism that resolves the local agent

**Lesson Learned**: Go module proxy is immutable. Once a version is cached with a particular module path, it cannot be changed. If you rename a module, you MUST bump the version — the old version is permanently poisoned. The `go.work` `use` directive does NOT override proxy validation for required dependencies.

**Files Changed**: 56 Go source files, `go.mod`, `agent/go.mod`, `go.work.sum`, `README.md`, `agent/README.md`, `docs/releasing.md`, `docs/progress.md`

**Next Steps**:
1. Merge `design/ollama-support` branch to master
2. Tag `agent/v0.2.0` + `v0.2.0` after merge
3. Push tags to both `superbuilders` and `private` remotes
4. Verify `go install github.com/superbuilders/clyde@v0.2.0` works
